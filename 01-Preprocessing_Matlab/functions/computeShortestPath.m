...
% between any two locations in the WP1 Starmaze environment.
% Requires Matlab 2021a for 'shortestpath' function and 
% 'interparc' function by John D'Errico (Matlab File Exchanger).
% 
% Input:
% toGoal indicates if this path ends at a known graph node (boolean)
% central_polygon is inner ring as one combined polyshape (polyshape)
% full_polyshape is full combined polyshape (polyshape)
% graph is representation of all start-goal connections (graph)
% graph_x, graph_y are x-/y-coordinates of the graph (float)
% start_node is identifier for start in graph (integer)
% goal_node is identifier for goal in graph (integer)
% end_x, end_y are x-/y-coordinates of end point (float)
% 
% Returns: 
% x_line, y_line are vectors with ideal x-/y-coordinates (float)
% ideal_length ideal path length values (float)
...
if toGoal || inGraph
    ...
    % check for path length of zero (interpolation produces error)
    length2=computePathLength(x2, y2);
    if length2==0
        xi_s2=x2; yi_s2=y2; isValid=false;
    else
        % interpolate last segment (using 'interparc' function by John D'Errico (Matlab File Exchanger))
        [xi_s2,yi_s2]=interpolateIdealPath(x2(end-1:end),y2(end-1:end),length2);
...
%% if end point of the path is NOT part of the graph
    ...
    % interpolate last segment (using 'interparc' function by John D'Errico (Matlab File Exchanger))
    [xi_s1,yi_s1]=interpolateIdealPath(x1_reduced(end-1:end),y1_reduced(end-1:end),length1_reduced);
    % check if last segment is valid (i.e., not outside starmaze area)
    ...
    % interpolate last segment (using 'interparc' function by John D'Errico (Matlab File Exchanger))
    [xi_s2,yi_s2]=interpolateIdealPath(x2_reduced(end-1:end),y2_reduced(end-1:end),length2_reduced);
    % check if last segment is valid (i.e., not outside starmaze area)
